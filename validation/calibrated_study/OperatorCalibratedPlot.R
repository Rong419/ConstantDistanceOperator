...
for (para.name in parameter.names) {
     assign(paste0("true.",para.name),read.table(file=paste0(folder.path,taxa,"taxa/truedata/",para.name,".txt"), sep="\t"))
...
for (i in 1:n.sim) {
   this.sim.df = read.table(file=paste0(folder.path,taxa,"taxa/logfiles/calibrated",taxa,"taxa_",i,".log"),sep="\t",header=T)
