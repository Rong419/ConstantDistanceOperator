...
public class Relaxed16TaxaBMA {
	...
	static void process(String dir) throws IllegalArgumentException, IllegalAccessException, IOException, XMLParserException {
		...
		LogAnalyser trace = new LogAnalyser(wdir + "/bma16.log", 0, false, false);
		...
		for (int i = 0; i < 30; i++) {
			quantilesX[i] = trace.getTrace("rateQuantiles." + (i+1));
		}
...
//		tree = new beast.util.TreeParser(newick="(((A:0.1,B:0.1),C:0.15):0.05,(D:0.1,E:0.1):0.1)", taxa=data, IsLabelledNewick=true);
		...
		for (int j = 0; j < 30; j++) {
			rateQuantiles.setValue(j, quantilesX[j][i]);
		}
		...
		mergewith.initByName("template", wdir + "/analysis16relaxedBMA.xml", "output", dir + "/analysis-out" + i + ".xml");
	...
	static String wdir = "/Users/remco/workspace/ConstantDistanceOperator/validation/quantiles";
		...
		File fin = new File(wdir + "/bma16.trees");
